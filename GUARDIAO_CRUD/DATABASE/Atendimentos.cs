...
namespace GUARDIAO_CRUD.DATABASE
{
  public class Atendimentos : DataBase
  {
    ...
    public List<GUARDIAO_STRUCTS.DATABASE.AtendimentoQuantidade> GetAtendimentosData()
    {
      string sql = "";
      List<GUARDIAO_STRUCTS.DATABASE.AtendimentoQuantidade> especialidade = null;
      DataSet ds = null;
      try
      {
        sql = "SELECT a.atendimento_status, COUNT(*) AS quantidade_atendimento ";
        sql += " FROM tblAtendimentos a";
        sql += " GROUP BY a.atendimento_status ";

        ds = ExecuteSelect(sql);
        if (ds == null || ds.Tables.Count == 0 || ds.Tables["T"].Rows.Count == 0)
          throw new Exception("Nenhum atendimento encontrado.");

        especialidade = new List<GUARDIAO_STRUCTS.DATABASE.AtendimentoQuantidade>();
        for (int i = 0; i < ds.Tables["T"].Rows.Count; i++)
        {
          var row = ds.Tables["T"].Rows[i];
          var atendimentoStatus = row["atendimento_status"].ToString();
          var quantidadeAtendimento = Convert.ToInt32(row["quantidade_atendimento"]);
          var descricaoStatus = GetDescricaoAtendimentoStatus(atendimentoStatus);

          var atendimento = new GUARDIAO_STRUCTS.DATABASE.AtendimentoQuantidade
          {
            AtendimentoStatus = atendimentoStatus,
            DescricaoStatus = descricaoStatus,
            QuantidadeAtendimento = quantidadeAtendimento
          };

          especialidade.Add(atendimento);
        }
      }
      catch (Exception ex)
      {
    ...
    public List<GUARDIAO_STRUCTS.DATABASE.AtendimentoQuantidade> GetAtendimentosData(int idUsuario)
    {
      string sql = "";
      List<GUARDIAO_STRUCTS.DATABASE.AtendimentoQuantidade> especialidade = null;
      DataSet ds = null;
    ...
    private string GetDescricaoAtendimentoStatus(string status)
    {
      switch (status)
      {
        case "PA":
          return "Pendente Atendimento";
        case "EA":
          return "Em Andamento";
        case "AF":
