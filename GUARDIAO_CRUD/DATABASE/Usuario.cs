...
namespace GUARDIAO_CRUD.DATABASE
{
  ...
  public class Usuario : DataBase
  {
    ...
    public GUARDIAO_STRUCTS.DATABASE.Usuario ValidarLoginUsuario(long usuario_cpf, string usuario_senha, int tipoUsuario)
    {
      ...
      try
      {
        ...
        if (tipoUsuario == 1)
        {
          if (usuario.especieUsuario_id == 5)
          {
            throw new Exception("USUÁRIO TIPO CLIENTE NÃO É PERMITIDO ENTRAR NOS SISTEMA WEB, BAIXE O APLICATIVO GUARDIÃO NA SUA LOJA DE APPS");
          }
        }
    ...
    public List<GUARDIAO_STRUCTS.DATABASE.UsuarioQuantidade> GetQuantidadeUsuario()
    {
      string sql = "";
      List<GUARDIAO_STRUCTS.DATABASE.UsuarioQuantidade> usuario = null;
      DataSet ds = null;
      try
      {
        sql = "SELECT CONVERT(DATE, usuario_dataCadastro) AS DataCriacao, COUNT(*) AS TotalAdvogados ";
        sql += "FROM tblUsuario ";
        sql += "WHERE especieUsuario_id = 4 ";
        sql += "AND usuario_dataCadastro >= DATEADD(DAY, -30, GETDATE()) ";
        sql += "GROUP BY CONVERT(DATE, usuario_dataCadastro) ";
        ...
        usuario = new List<GUARDIAO_STRUCTS.DATABASE.UsuarioQuantidade>();
        int ContadorAdvogado = 0;  // Inicializa o contador acumulado fora do loop

        for (int i = 0; i < ds.Tables["T"].Rows.Count; i++)
        {
          var row = ds.Tables["T"].Rows[i];
          var dataCriacao = row["DataCriacao"].ToString();
          var formattedDataCriacao = DateTime.Parse(dataCriacao).ToString("dd/MM/yyyy");
          var totalAdvogados = Convert.ToInt32(row["TotalAdvogados"]);

          // Atualiza o contador acumulado somando o total de advogados do dia atual
          ContadorAdvogado += totalAdvogados;

          usuario.Add(new GUARDIAO_STRUCTS.DATABASE.UsuarioQuantidade
          {
            DataCriacao = formattedDataCriacao,
            TotalAdvogados = totalAdvogados,
            ContadorAdvogados = ContadorAdvogado // Adiciona o contador acumulado
          });
        }
      }
      catch (Exception ex)
    ...
    public List<GUARDIAO_STRUCTS.DATABASE.UsuarioQuantidadeAdvogado> GetQuantidadeUsuario(int idUsuario)
    {
      string sql = "";
      List<GUARDIAO_STRUCTS.DATABASE.UsuarioQuantidadeAdvogado> usuario = null;
      DataSet ds = null;
      try
      {
        sql = "SELECT CONVERT(DATE, atendimento_dataInicio) AS DataAtendimento, COUNT(*) AS TotalAtendimentos";
        sql += " FROM tblAtendimentos ";
        sql += " WHERE advogado_id = " + idUsuario;
        sql += " AND atendimento_dataInicio >= DATEADD(DAY, -30, GETDATE()) ";
        sql += " GROUP BY CONVERT(DATE, atendimento_dataInicio) ";
        sql += " ORDER BY DataAtendimento ";

        ds = ExecuteSelect(sql);
        if (ds == null)
          throw new Exception("Nenhuma especialidade encontrada.");

        usuario = new List<GUARDIAO_STRUCTS.DATABASE.UsuarioQuantidadeAdvogado>();
        int ContadorAdvogado = 0;  // Inicializa o contador acumulado fora do loop

        for (int i = 0; i < ds.Tables["T"].Rows.Count; i++)
        {
          var row = ds.Tables["T"].Rows[i];
          var dataCriacao = row["DataAtendimento"].ToString();
          var formattedDataCriacao = DateTime.Parse(dataCriacao).ToString("dd/MM/yyyy");
          var totalAdvogados = Convert.ToInt32(row["TotalAtendimentos"]);

          // Atualiza o contador acumulado somando o total de advogados do dia atual
          ContadorAdvogado += totalAdvogados;

          usuario.Add(new GUARDIAO_STRUCTS.DATABASE.UsuarioQuantidadeAdvogado
          {
            DataAtendimento = formattedDataCriacao,
            TotalAtendimentos = totalAdvogados,
            ContadorAdvogados = ContadorAdvogado // Adiciona o contador acumulado
          });
        }
      }
    ...
    public List<GUARDIAO_STRUCTS.DATABASE.UsuarioAdvogadoAtivo> GetAllUsuarioAdvogadoAtivo()
    {
      string sql = "";
      DataSet ds = null;
      List<GUARDIAO_STRUCTS.DATABASE.UsuarioAdvogadoAtivo> usuarios = null;
      try
      {
        sql = "SELECT usuario_id,usuario_nome, usuario_ativo, usuario_email, usuario_oab, usuario_telefone,usuario_dataCadastro FROM tblusuario WHERE especieUsuario_id = 4 AND usuario_ativo = 0 ORDER BY usuario_dataCadastro DESC";

        ds = ExecuteSelect(sql);
        if (ds == null)
          throw new Exception("Nenhum registro encontrado para os parâmetros informados.");
        usuarios = new List<GUARDIAO_STRUCTS.DATABASE.UsuarioAdvogadoAtivo>();
        for (int i = 0; i < ds.Tables["T"].Rows.Count; i++)
        {
          usuarios.Add(ds.Tables["T"].Rows[i].DataRowToBoject<GUARDIAO_STRUCTS.DATABASE.UsuarioAdvogadoAtivo>());
        }
      }
      catch (Exception)
      {
    ...
    public List<GUARDIAO_STRUCTS.DATABASE.UsuarioAdvogadoAtendimentoAtivo> GetAllUsuarioAdvogadoAtivo(int idUsuario)
    {
      string sql = "";
      DataSet ds = null;
      List<GUARDIAO_STRUCTS.DATABASE.UsuarioAdvogadoAtendimentoAtivo> usuarios = null;
      try
      {
        sql =  "SELECT ";
        ...
        sql += " ,tblSolicitacaoAtendimento.solicitacaoAtendimento_id";
        sql += " ,tblUsuario.usuario_nome";
        ...
        sql += " WHERE tblSolicitacaoAtendimento.advogado_id = " + idUsuario;
        sql += " AND tblSolicitacaoAtendimento.solicitacaoAtendimento_status = 'PA'";

        ds = ExecuteSelect(sql);
        if (ds == null)
          throw new Exception("Nenhum registro encontrado para os parâmetros informados.");
        usuarios = new List<GUARDIAO_STRUCTS.DATABASE.UsuarioAdvogadoAtendimentoAtivo>();
        for (int i = 0; i < ds.Tables["T"].Rows.Count; i++)
        {
          usuarios.Add(ds.Tables["T"].Rows[i].DataRowToBoject<GUARDIAO_STRUCTS.DATABASE.UsuarioAdvogadoAtendimentoAtivo>());
        }
      }
      catch (Exception)
      {
    ...
    public bool AtivaUsuario(int idUsuario)
    {
      string sql = "";
      long reg = 0;
      bool usuarioAtivado = false;
      try
      {
        sql = "UPDATE TBLUSUARIO SET USUARIO_ATIVO = 1 WHERE USUARIO_ID = " + idUsuario;
