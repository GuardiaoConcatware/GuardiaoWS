...
namespace GUARDIAO_CRUD.DATABASE
{
  public class Especialidade : DataBase
  {
    ...
    public List<GUARDIAO_STRUCTS.DATABASE.EspecialidadeQuantidade> GetEspecialidadesData()
    {
      string sql = "";
      ...
      try
      {
        sql = "SELECT c.especialidade_nome, COUNT(*) AS quantidade_especialidades ";
        sql += " FROM tblUsuario a";
        sql += " INNER JOIN tblEspecialidadeUsuario b ON a.usuario_id = b.usuario_id ";
        sql += " INNER JOIN tblEspecialidade c ON b.especialidade_id = c.especialidade_id ";
        sql += " GROUP BY c.especialidade_nome ";

        ds = ExecuteSelect(sql);
        if (ds == null)
          throw new Exception("Nenhuma especialidade encontrada.");

        especialidade = new List<GUARDIAO_STRUCTS.DATABASE.EspecialidadeQuantidade>();
        for (int i = 0; i < ds.Tables["T"].Rows.Count; i++)
        {
          especialidade.Add(ds.Tables["T"].Rows[i].DataRowToBoject<GUARDIAO_STRUCTS.DATABASE.EspecialidadeQuantidade>());
        }
    ...
    public List<GUARDIAO_STRUCTS.DATABASE.EspecialidadeQuantidade> GetEspecialidadesDataByAdvogado(int idUsuario)
    {
      string sql = "";
      List<GUARDIAO_STRUCTS.DATABASE.EspecialidadeQuantidade> especialidade = null;
      DataSet ds = null;
      try
      {
        ...
        sql += " INNER JOIN tblEspecialidadeUsuario b ON a.usuario_id = b.usuario_id ";
        sql += " INNER JOIN tblEspecialidade c ON b.especialidade_id = c.especialidade_id";
        sql += " WHERE a.usuario_id = " + idUsuario;
        sql += " GROUP BY c.especialidade_nome ";
